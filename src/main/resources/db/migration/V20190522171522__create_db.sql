...
create table lightning_school.group (
    id_group integer not null auto_increment,
    libelle_group varchar(255),
    primary key (id_group)
);

create table lightning_school.group_exercice (
    id_group_exercice integer not null auto_increment,
    mark float,
    path_rendu varchar(255),
    exercice_id_exercice integer,
    group_id_group integer,
    primary key (id_group_exercice)
);

create table lightning_school.group_users (
...
create table lightning_school.section_groups (
    section_id_section integer not null,
    groups_id_cours integer not null
);
...
create table lightning_school.user_groups (
...
alter table lightning_school.group_exercice
    add constraint FKg9pcjk0ecs606fpku11qt7dex
        foreign key (exercice_id_exercice)
            references lightning_school.exercice (id_exercice);

alter table lightning_school.group_exercice
    add constraint FK85wqh6oh8yy1g6xg6avfwkwxj
        foreign key (group_id_group)
            references lightning_school.group (id_group);

alter table lightning_school.group_users
    add constraint FKcmhbce939bft6bntlbxah1yy7
        foreign key (users_user_id)
            references lightning_school.user (user_id);

alter table lightning_school.group_users
    add constraint FKb0c30mifym79iw3h0x53ach3v
        foreign key (group_id_group)
            references lightning_school.group (id_group);
...
alter table lightning_school.section_groups
    add constraint FKgieffan7fmwf53bsr826jnin3
        foreign key (groups_id_cours)
            references lightning_school.cours (id_cours);

alter table lightning_school.section_groups
    add constraint FKmqo6s8c5ccpyv99b5m8mdm9mg
...
alter table lightning_school.user_groups
    add constraint FKk05l594vfp5cpqw7f4u5g8mhu
        foreign key (groups_id_group)
            references lightning_school.group (id_group);

alter table lightning_school.user_groups
    add constraint FKec2pr0urk1qs1oghl73jg6kbv
        foreign key (user_user_id)
            references lightning_school.user (user_id);
